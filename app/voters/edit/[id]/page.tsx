...
export default function EditVoterPage() {
  ...
  const { register, handleSubmit, formState: { errors }, setValue } = useForm<VoterForm>()
  ...
  const loadData = async () => {
    try {
      ...
      if (profileData) {
        ...
        // If not super admin, only allow editing own data
        if (profileData.role !== 'super_admin') {
          query = query.eq('created_by', user.id)
        }

        const { data: voterData } = await query.single()
        
        if (voterData) {
          // Populate form with existing data
          Object.keys(voterData).forEach(key => {
            if (key !== 'id' && key !== 'created_at' && key !== 'updated_at' && key !== 'created_by') {
              setValue(key as keyof VoterForm, voterData[key] || '')
            }
          })
        } else {
          toast.error('Électeur non trouvé ou accès non autorisé')
          router.push('/voters')
        }
      }
    } catch (error) {
  ...
  const onSubmit = async (data: VoterForm) => {
    ...
    try {
      ...
      // If not super admin, only allow updating own data
      ...
      if (error) {
        ...
        toast.success('Électeur mis à jour avec succès!')
        router.push('/voters')
      }
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="p-3 bg-gradient-to-r from-accent-violet to-accent-blue rounded-full">
              <Edit className="h-8 w-8 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white">Modifier l'Électeur</h1>
              <p className="text-primary-300">Modifiez les informations de l'électeur</p>
            </div>
          </div>
        </div>
